...
| --- | --- | --- |
| Main/default 0.4.6f13 | IL2CPP | Version 2.0.1 builds against the matching generated assemblies; live-game verification is tracked in `docs/HANDOVER.md`. |
...
The build output is `bin/Il2Cpp/ImprovedPackagers.dll`. The legacy `ImprovedPackagers.csproj` remains available for the older Mono workflow; the current IL2CPP build uses `ImprovedPackagers.Il2Cpp.csproj` and targets `net6.0`.
...
- If the game loads two Improved Packagers DLLs, remove both and install only the intended version.
